...
# --- to do with training --- #
def train_epoch(epoch, samples, labels, sess, Z, X, D_loss, G_loss, D_solver, G_solver, batch_size,
                use_time, D_rounds, G_rounds, seq_length, latent_dim, num_signals):
    """
...
def GAN_solvers(D_loss, G_loss, learning_rate, batch_size, total_examples, l2norm_bound, batches_per_lot, sigma, dp=False):
    ...
    if dp:
        ...
        # the trick is that we need to calculate the gradient with respect to each example in the batch, during the DP SGD step
        ...
        D_solver = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.1).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
        # D_solver = tf.compat.v1.train.AdagradOptimizer(learning_rate, initial_accumulator_value=0.1, use_locking=False, name='Adagrad')
        # D_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
        # D_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)
        # D_solver = tfa.optimizers.RectifiedAdam(lr=1e-3, total_steps=10000, warmup_proportion=0.1, min_lr=1e-5).minimize(D_loss_mean_over_batch, var_list=discriminator_vars)

        priv_accountant = None
    G_loss_mean_over_batch = tf.reduce_mean(G_loss)
    # G_solver = tf.compat.v1.train.AdamOptimizer().minimize(G_loss_mean_over_batch, var_list=generator_vars)
    # G_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(G_loss_mean_over_batch, var_list=generator_vars)
    G_solver = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.1).minimize(G_loss_mean_over_batch, var_list=generator_vars)
    # G_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(G_loss, var_list=generator_vars)
...
def generator(z, hidden_units_g, seq_length, batch_size, num_signals, reuse=False, parameters=None, learn_scale=True):
    ...
    with tf.compat.v1.variable_scope("generator") as scope:
        ...
        if parameters is None:
            W_out_G_initializer = tf.compat.v1.truncated_normal_initializer()
            b_out_G_initializer = tf.compat.v1.truncated_normal_initializer()
            scale_out_G_initializer = tf.compat.v1.constant_initializer(value=1.0)
        ...
        W_out_G = tf.compat.v1.get_variable(name='W_out_G', shape=[hidden_units_g, num_signals], initializer=W_out_G_initializer)
        b_out_G = tf.compat.v1.get_variable(name='b_out_G', shape=num_signals, initializer=b_out_G_initializer)
        scale_out_G = tf.compat.v1.get_variable(name='scale_out_G', shape=1, initializer=scale_out_G_initializer, trainable=learn_scale)
        # inputs
        inputs = z
        ...
        output_3d = tf.compat.v1.reshape(output_2d, [-1, seq_length, num_signals])

    return output_3d


# def discriminator(x, hidden_units_d, seq_length, batch_size, reuse=False, parameters=None, batch_mean=False):     # --> Original do MAD GAN
