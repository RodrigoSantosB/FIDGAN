...
def swat(seq_length, seq_step, num_signals, data_path, randomize=False):
    """ Load and serialise """
    ...
    np.load = np_load_old
    print('load swat_train from .npy')
...
def swat_validation(seq_length, seq_step, num_signals, data_path, randomize=False):
    """ Load and serialise """
    ...
    np.load = np_load_old
    print('load swat_validation from .npy')
...
def swat_test(seq_length, seq_step, num_signals, data_path, randomize=False):
    """ Load and serialise """
    # test = np.load('./data/swat_a.npy')
    # print('Loaded swat_a from .npy')
    # test = np.loadtxt(open('./data/swat_a.csv'), delimiter=',')
...
def wadi(seq_length, seq_step, num_signals, data_path, randomize=False):
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    train = np.load(data_path)
    np.load = np_load_old
    print('load wadi_train from .npy')
...
def wadi2(seq_length, seq_step, num_signals, data_path, randomize=False):
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    train = np.load(data_path)
    np.load = np_load_old
    ...
    # samples = train[259200:, 0:n-1]  # normal
    # labels = train[259200:, n-1]
    samples = train[21600:, 0:n-1]  # normal
    labels = train[21600:, n-1]
    # samples = train[:, 0:n-1]  # normal
    # labels = train[:, n-1]
    #############################
    #############################
    ...
    # # Only for the autoencoder, delete it for training the RGAN -------------
    # m2, n2 = samples.shape
    # for i in range(n2):
    #     A = max(samples[:, i])
    #     if A != 0:
    #         samples[:,i] = samples[:,i]-np.min(samples[:,i]) 
...
def wadi_test(seq_length, seq_step, num_signals, data_path, randomize=False):
...
def wadi3(seq_length, seq_step, num_signals, data_path):
    ...
    np.load = np_load_old
    print('load wadi_train from .npy')
    m, n = train.shape  # m=449909, n=35 (562387)
...
def wadi_test3(seq_length, seq_step, num_signals, data_path):
    # test = np.load('./data/kdd99_test.npy')
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    test = np.load(data_path)
    np.load = np_load_old
    print('load wadi_test from .npy')
    m, n = test.shape  # m1=494021, n1=35

    samples = test[:, 0:n - 1]
    labels = test[:, n - 1]        # the last colummn is label
...
def nslkdd(seq_length, seq_step, num_signals, data_path):
    # train = np.load('./data/kdd99_train.npy')
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    train = np.load(data_path)
    np.load = np_load_old
    print('load nslkdd_train from .npy')
    m, n = train.shape  # m=449909, n=35 (562387)
    # normalization
    for i in range(n - 1):
        # print('i=', i)
    ...
    samples = train[:, 0:n - 1]
    labels = train[:, n - 1]  # the last colummn is label
    # samples = train[int(m*0.2):, 0:n-1]
    # labels = train[int(m*0.2):, n-1]    # the last colummn is label
    #############################
    ############################
    # -- apply PCA dimension reduction for multi-variate GAN-AD -- #
    from sklearn.decomposition import PCA
...
def nslkdd_test(seq_length, seq_step, num_signals, data_path):
    # test = np.load('./data/kdd99_test.npy')
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    test = np.load(data_path)
    np.load = np_load_old
    print('load nslkdd_test from .npy')
    m, n = test.shape  # m1=494021, n1=35
    for i in range(n - 1):
        B = max(test[:, i])
        if B != 0:
...
def nslkdd2(seq_length, seq_step, num_signals, data_path):
    # train = np.load('./data/kdd99_train.npy')
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    train = np.load(data_path)
...
def nslkdd_test2(seq_length, seq_step, num_signals, data_path):
    # test = np.load('./data/kdd99_test.npy')
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    test = np.load(data_path)
...
def kdd99(seq_length, seq_step, num_signals, data_path):
    # train = np.load('./data/kdd99_train.npy')

    # Because of the swat dataset
    np_load_old = np.load
    np.load = lambda *a,**k: np_load_old(*a, allow_pickle=True, **k)
    train = np.load(data_path)
    np.load = np_load_old
...
def kdd99_validation(seq_length, seq_step, num_signals):
    test = np.load('./data/kdd99_validation.npy')
    print('load kdd99_validation from .npy')
...
def kdd99_test(seq_length, seq_step, num_signals, data_path):
    # test = np.load('./data/kdd99_test.npy')
...
def get_data(data_type, seq_length, seq_step, num_signals, sub_id, eval_single, eval_an, data_path):
    """
    ...
    if data_type == 'load':
        ...
        # samples, labels = swat(seq_length, seq_step, num_signals, data_path)
        samples, labels = kdd99(seq_length, seq_step, num_signals, data_path)
    elif data_type == 'swat_validation':
        samples, labels, index = swat_validation(seq_length, seq_step, num_signals, data_path)
    elif data_type == 'swat_test':
        # samples, labels, index = swat_test(seq_length, seq_step, num_signals, data_path)
        samples, labels, index = kdd99_test(seq_length, seq_step, num_signals, data_path)
    elif data_type == 'kdd99':
        samples, labels = kdd99(seq_length, seq_step, num_signals, data_path)
    elif data_type == 'kdd99_validation':
        samples, labels, index = kdd99_validation(seq_length, seq_step, num_signals)
        ...
        samples, labels, index = kdd99_test(seq_length, seq_step, num_signals, data_path)
    elif data_type == 'nslkdd':
        # samples, labels = nslkdd2(seq_length, seq_step, num_signals, data_path)
        samples, labels = kdd99(seq_length, seq_step, num_signals, data_path)
